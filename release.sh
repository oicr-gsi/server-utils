...
Server utils uses semantic versioning. Valid release types: major, minor, patch"
    exit 3
    ;;
esac
...
git commit -a -m "Server utils ${RELEASE_VERSION} release"
git tag -a v${RELEASE_VERSION} -m "Server utils ${RELEASE_VERSION} release"
mvn clean install
mvn deploy
