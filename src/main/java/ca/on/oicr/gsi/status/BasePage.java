...
public abstract class BasePage {
  private static final Header CSS =
      Header.css(
  ...
  /** Write the HTML body of the page; the header and navigation links have already been supplied */
  ...
  public final void renderPage(OutputStream output) {
    ...
    try {
      ...
      if (autorefresh) writer.writeEmptyElement("meta http-equiv=\"refresh\" content=\"5\"");

      CSS.append(writer);
      Stream.concat(headers(), server.headers())
          .forEach(
              header -> {
                try {
