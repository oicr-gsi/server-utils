...
public final class Pair<T, U> {
  /** Convert a consumer of two items into a consumer of pairs of the same types of items. */
  public static <T, U> Consumer<Pair<T, U>> consume(BiConsumer<? super T, ? super U> consumer) {
    return pair -> pair.accept(consumer);
  }

  /**
   * Create a stateful function that transforms an item into a pair with the first element being the
   ...
   * index of this item.
   *
   * <p>This is useful to number items using {@link Stream#map(Function)}
   *
   * @return
   */
  ...
  public static <T, U> Predicate<Pair<T, U>> predicate(
      BiPredicate<? super T, ? super U> predicate) {
    return pair -> pair.test(predicate);
  }

  /** Create a transformation function for pairs from a function that takes two inputs */
  ...
  public <V, W> Pair<V, W> map(
      Function<? super T, ? extends V> firstFunction,
